...
//--METODOS DE CLASE
	...
	public static function MostrarTablaProductos()
    {
        try
        {
            $user = "root";
            ...
            $obj = $db->prepare("SELECT * FROM `productos`");
            $obj->execute();

            $table = '<table border="1" style="border-collapse:collapse"><tr><td>ID</td><td>Codigo barra</td><td>Nombre</td><td>Foto</td></tr>';
            while($row = $obj->fetch(PDO::FETCH_OBJ))
            {
                $table .= "<tr><td>" . $row->id . "</td><td>" . $row->codigo_barra . "</td><td>" . $row->nombre . "</td><td><img src='./fotos/" . $row->path_foto . "' style='width:100px;height:100px;'></td></tr>";
            }
            $table .= "</table>";
            return $table;
        }
